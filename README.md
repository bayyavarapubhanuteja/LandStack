...
The repo ships a `Dockerfile` and `render.yaml` that build the React site and serve it from the
FastAPI container, so the whole app runs as **one service on one URL** — no API URL, CORS or
cross-service configuration to set up.

1. Render → **New → Blueprint** → pick this repository.
2. Approve the sync. Render builds the Dockerfile and deploys `landstack`.
3. Open the service URL. The demo dataset seeds itself on first boot.

`JWT_SECRET` is generated by Render. On the free plan the service sleeps after 15 minutes idle
(first request then takes ~50s) and its disk is ephemeral, so submitted service requests are lost
on restart — the 18 demo parcels and the demo accounts are re-seeded automatically. For durable
data, attach a Postgres instance and set `DATABASE_URL`.
